...
const productSchema = z.object({
    name: z.string().min(3, { message: 'The name has to be minimum of length 3' }), // Fixed message
    category: z.string().min(1, { message: 'Category is required' }),
    price: z.number().min(1, { message: 'The price has to be bigger than 0' }),
    smallDescription: z.string().min(10, { message: 'Please summarize your text in 10 characters minimum' }), // Fixed message
    description: z.string().min(10, { message: 'Description is required' }),
    images: z.array(z.string()).min(1, { message: "At least one image is required" }), // Fixed validation
    productFile: z.string().min(1, { message: "Please upload your product file" })
})

const userSettingSchema = z.object({
    firstName : z.string().min(3, { message : "Minimum 3 length of 3 required"}).or(z.literal("")).optional(),
...
export async function SellProduct(preState: any, formData: FormData) {
  ...
  data: {
    ...
    images: validateFields.data.images, // should be string[]
